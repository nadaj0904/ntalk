# [AI 개발 지침서] 보험영업지원 시스템(Ntalk) 복제 및 개발 가이드

본 문서는 `Dev_ntalk_project` 프로젝트의 기술 아키텍처, UI/UX 디자인 시스템, 백엔드 및 프론트엔드 개발 표준을 기반으로 작성되었습니다. 새로운 프로젝트를 본 프로젝트와 **동일한 기술 스택 및 개발 방식**으로 개발할 때, AI 코딩 어시스턴트가 반드시 준수해야 하는 지침을 정의합니다.
...
### 백엔드 (Backend)
- **Language**: Java 17
- **Framework**: Spring Boot 4.0.5 (또는 3.x 호환 버전)
- **Build Tool**: Maven (WAR 패키징 구성, `pom.xml` 빌드 이름 `ntalk` 지정)
- **Database**: PostgreSQL (AWS Lightsail 또는 로컬 환경)
- **ORM / SQL Mapper**: MyBatis (`mybatis-spring-boot-starter` 4.0.1)
- **Libraries**:
...
- **Naming**: 테이블명은 `TB_NTALK_[비즈니스명]` 패턴을 따릅니다.
